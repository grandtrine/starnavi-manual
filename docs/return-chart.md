...
- **ネイタル(N)／ディレクション(D)／リターン(R)** タブ：各リングの天体表（天体・サイン・度数・赤緯・ハウス・支配ハウス）を表示します。右パネルの天体表に表示する天体は、3 つのタブとも表示設定（天体表示）の **N：ネイタル** の内容に従います（円盤の外円とは異なります。「表示モードを切り替える」の注意を参照）。
...
- 未保存の編集のままチャートを表示して印刷すると、印刷ヘッダーに「**未保存の出生データ**」と表示されます。
