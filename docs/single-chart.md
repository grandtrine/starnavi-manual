...
- データタブに表示されるのは、選択している「**プリセット**」で選んでいる項目だけです。各タブに表示される内容は次のとおりです。
    ...
    - **分析タブ**: 月相 / 三区分 / 四区分 / 二区分 / 半球の強調 / ゴークランセクターの天体 / ステリウム / 特殊度数などを表示します。さらにチャート詳細として、「タイトなコンジャンクション/オポジション」「その他のタイトなアスペクト」「最多アスペクト天体」「アンギュラー天体」「ASC の支配星」「エッセンシャルディグニティ」「ドデカテモリー」「アンティシオン」「ミューチュアルリセプション」を表示します（一部、プランによって表示されないものがあります）。チャート詳細の前には、支配星をたどる矢印をチャートに重ねた「**ディスポジター**」の図を表示します（Pro 以上。見方は [ディスポジター](dispositor.md) の章）。
...
![編集ボタンで開く出生データの編集フォーム。氏名・生年月日・出生時刻・出生地を修正して再計算し、上書き保存/別名で保存で残せる](assets/single-chart-06-edit-recalc.jpg)
