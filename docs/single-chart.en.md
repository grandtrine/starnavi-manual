...
![The chart type selector above the chart (red box). You can choose Normal, Dragon Head, or Dragon Tail](assets/single-chart-07-dragon-selector.png)

### Steps

1. Open the **Chart Type** selector above the chart (below the birth data display), which reads **Normal**.
2. Choosing **Dragon Head** or **Dragon Tail** switches the chart and the data tabs immediately (there is no need to recalculate).
3. Returning it to **Normal** brings back the original natal chart.
...
1. Press **Show Chart** and the chart appears on the left with the data on the right (below, on a smartphone).
2. On the right, switch tabs to see the various sets of data.

### Notes

![The chart display. Clicking Neptune pops up the list of aspects to that planet (the other planet, the type, the orb, and whether it is applying or separating)](assets/single-chart-03-wheel-popup.jpg){ width="460" }
...
- The data tabs show only the items selected in the **preset** you have chosen. Each tab contains the following.
    ...
    - **Aspects tab**: shows the aspect grid, a list of aspects in orb order, and the aspect patterns. Below the grid is a **list, ordered from the tightest (smallest orb) aspect, with keywords**. Below that, **aspect patterns (special figures formed by several planets) are detected automatically and listed**. In the aspect list and the grid, applying aspects are marked with a **minus sign** and separating aspects with a **plus sign**. At the bottom of the tab, planets with no aspect to the main planets (and planets with no major aspect) are listed when there are any.
      ![An example of the Aspects tab. Hovering over a cell of the grid shows the meaning of the aspect, and below it the aspects are listed from the tightest, with keywords](assets/single-chart-02-panels.jpg)
    - **Analysis tab**: shows the summaries (moon phase, modality, element, polarity, quadrant emphasis, Gauquelin sector planets, stellium, and critical degrees), the **Dispositors** chart, which overlays arrows tracing each planet's ruler on the wheel (Pro and above), and **Chart Details** (tight aspects, most aspected planets, angular planets, ASC ruler, essential dignity including the almuten (Pro and above), dodecatemoria, antiscion, and mutual receptions). See the [Analysis Tab](analysis-tab.md) chapter for each item, and the [Dispositors](dispositor.md) chapter for the dispositor chart.
...
!!! tip "Meanings are available on the spot"
...
!!! note "The orbs for aspect patterns"
...
![The stepping controls (play, 1 step, interval, speed, backward)](assets/triple-chart-04-step.jpg)
...
1. The **Print** button prints the chart and the data (**Basic and above**).
2. Clicking the wheel enlarges it, and from the enlarged view you can save the image with **PNG** (PNG saving is **Basic and above**).
...
1. Press the **Display Settings** button on the chart screen.
2. Switch planets, aspects, orbs, the house system, the progression method, and so on, on the spot.
3. The chart and the data in the right panel are redrawn immediately.
